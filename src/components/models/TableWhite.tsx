...
interface TableWhitePops {
  [key: string]: any;
}

export const TableWhite: React.FC<TableWhitePops> = (props) => {
  const { nodes, materials } = useGLTF('../../../public/models/tableWhite/scene.gltf') as unknown as {
    nodes: { [key: string]: Group };
    materials: { [key: string]: Material };
  };
  const tableWhiteRef = useRef<Group>(null);

  useFrame(() => {
    if (tableWhiteRef.current) {
    ...
    <group {...props} ref={tableWhiteRef} dispose={null}>
      <group position={[0, 22.582, 0]} rotation={[-Math.PI / 2, 0, 0]} scale={[1.17, 1, 1.17]}>
        <group position={[-74.283, 10, 70.223]}>
          <mesh geometry={(nodes.Esme_Coffee_Table_with_2_Drawers_Ash_WHITE_0 as unknown as THREE.Mesh).geometry} material={materials.WHITE} />
          <mesh geometry={(nodes.Esme_Coffee_Table_with_2_Drawers_Ash_ASH_0 as unknown as THREE.Mesh).geometry} material={materials.material} />
        </group>
      </group>
